...
class ContractService extends Service {
  ...
  async onReorg(height) {
    ...
    `, {replacements: {transferABIId: TransferABI.id, height, zeroHash: Buffer.alloc(32)}})
  }
  ...
  async _syncContracts() {
    ...
    if (contractsToRemove.length) {
      ...
      await this.#QRC721Token.destroy({where: {contractAddress: {[$in]: addresses}}})
      await this.#QRC721.destroy({where: {contractAddress: {[$in]: addresses}}})
      await this.#QRC20Balance.destroy({where: {contractAddress: {[$in]: addresses}}})
      await this.#QRC20.destroy({where: {contractAddress: {[$in]: addresses}}})
      await this.#ContractTag.destroy({where: {contractAddress: {[$in]: addresses}}})
      await this.#Contract.destroy({where: {address: {[$in]: addresses}}})
    }
