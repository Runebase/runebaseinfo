...
const {gt: $gt, in: $in, or: $or} = Sequelize.Op
...
class TransactionService extends Service {
  ...
  async start() {
    ...
    `, {replacements: {height: this.#tip.height}})
  ...
  async onReorg(height) {
    ...
    `, {replacements: {minHeight: Math.max(height, this.chain.lastPoWBlockHeight)}})
    await this.#db.query(`
    ...
    `, {replacements: {height, lastPoWBlockHeight: this.chain.lastPoWBlockHeight}})
  ...
  async _processBlock(block) {
    ...
    if (this.#synced) {
      ...
      if (mempoolTransactions.length) {
        ...
        `, {replacements: {blockHeight: block.height, ids: _ids}})
        await this.#db.query(`
  ...
  async processTxos(transactions) {
    ...
    for (let {type, data} of addressMap.values()) {
      addressItems.push({type: this.#Address.parseType(type), data})
    }
    if (addressItems.length) {
      let existingAddresses = await this.#Address.findAll({
        where: {[$or]: addressItems.map(({type, data}) => ({type, data}))},
    ...
    await Promise.all([
      ...
      ...mappingObjects.length ? [
        this.#TransactionOutputMapping.bulkCreate(mappingObjects, {validate: false})
      ] : []
    ])
    await this.#db.query(`
  ...
  async processBalanceChanges({block, transactions}) {
    ...
    if (transactions) {
      ...
      if (block) {
        ...
        `, {replacements: {blockHeight: block.height}})
      }
      filter = 'transaction_id BETWEEN :minId AND :maxId'
      replacements = {minId: transactions[0]._id, maxId: transactions[transactions.length - 1]._id}
    } else {
  ...
  async removeReplacedTransactions(tx) {
    ...
    for (let input of tx.inputs) {
      ...
      inputTxos.push({transactionId: item._id, outputIndex: input.outputIndex})
    }
  ...
  async _removeMempoolTransaction(id) {
    ...
    if (inputs.length) {
      await this.#TransactionOutput.update(
        {inputId: 0, inputIndex: 0xffffffff},
        {where: {[$or]: inputs.map(i => ({transactionId: i.outputId, outputIndex: i.outputIndex}))}}
      )
    }
    await Promise.all([
    ...
    let txRecord = await this.#Transaction.findOne({where: {_id: id}, attributes: ['id']})
    if (txRecord) {
      await this.#Witness.destroy({where: {transactionId: txRecord.id}})
      await this.#Transaction.destroy({where: {_id: id}})
    }
  }
