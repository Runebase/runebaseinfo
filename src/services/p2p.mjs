...
class P2PService extends Service {
  #outgoingTransactions = new LRUCache({max: 100})
  #blockCache = null
  #pool = null
  #peer = null
  #peers = []
  ...
  constructor(options) {
    ...
    this._initP2P()
    this._initPubSub()
    this.#blockCache = new LRUCache({max: 32, ttl: 5 * 60 * 1000})
  }
  ...
  _initCache() {
    this.#inventories = new LRUCache({max: 1000})
  }

  _initP2P() {
    this.#maxPeers = 60
    this.#configPeers = [{ip: {v4: '127.0.0.1'}, port: parseInt(process.env.P2P_PORT) || 9433}]
