    ...
    try {
      ...
      if (headers.length === 0) {
        ...
        for (let header of transformedHeaders) {
          if (
            this.#lastHeader.height > 5000
            && Buffer.compare(this.#lastHeader.hash, header.prevHash) !== 0
          ) {
            this.logger.warn(
              `Header Service: headers not in order at height ${this.#lastHeader.height}:`,
              `${this.#lastHeader.hash.toString('hex')} -and- ${header.prevHash.toString('hex')};`,
              'treating as reorg'
            )
            this.#reorging = true
            this.emit('reorg')
