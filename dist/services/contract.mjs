...
import { Address, OutputScript, Solidity, Hash } from '../lib/index.mjs';
...
class ContractService extends Service {
  ...
  async onBlock(block) {
    if (block.height === 0) {
      for (let x of [0x80, 0x81, 0x82, 0x83, 0x84]) {
        ...
        await this.#ContractCode.bulkCreate([{
          sha256sum,
          code
        }], {
      ...
      for (let i = 0; i < transaction.outputs.length; ++i) {
        ...
        if (output.scriptPubKey.type === OutputScript.EVM_CONTRACT_CREATE) {
          ...
          let owner = new Address({
            type: [null, Address.PAY_TO_PUBLIC_KEY_HASH, Address.PAY_TO_SCRIPT_HASH, Address.PAY_TO_WITNESS_SCRIPT_HASH, Address.PAY_TO_WITNESS_KEY_HASH][output.scriptPubKey.senderType],
  ...
  async onReorg(height) {
    ...
    await this.#db.query(`
      ...
      INNER JOIN (
        ...
        WHERE topic4 IS NOT NULL AND topic1 = :transferABIId
        GROUP BY address, topic4
      ) results ON log._id = results._id
      WHERE receipt._id = log.receipt_id AND receipt.block_height > :height AND log.topic2 != :zeroHash
      ON DUPLICATE KEY UPDATE holder = VALUES(holder)
    `, {
      replacements: {
        transferABIId: TransferABI.id,
        height,
        zeroHash: Buffer.alloc(32)
      }
    });
  }
  async onSynced() {
  ...
  async _syncContracts() {
    ...
    if (contractsToRemove.length) {
      let addresses = contractsToRemove.map(addr => Buffer.from(addr, 'hex'));
      await this.#QRC721Token.destroy({
        where: {
          contractAddress: {
            [$in]: addresses
          }
      ...
      await this.#QRC20Balance.destroy({
        where: {
          contractAddress: {
            [$in]: addresses
          }
      ...
      await this.#ContractTag.destroy({
        where: {
          contractAddress: {
            [$in]: addresses
          }
  ...
  async _createContract(address, vm) {
    ...
    if (isQRC721(code)) {
      let [nameResult, symbolResult, totalSupplyResult] = await this._batchCallMethods([{
        address,
        abi: Solidity.qrc721ABIs.find(abi => abi.name === 'name')
      }, {
        address,
        abi: Solidity.qrc721ABIs.find(abi => abi.name === 'symbol')
      }, {
        address,
        abi: Solidity.qrc721ABIs.find(abi => abi.name === 'totalSupply')
      }]);
      try {
        let [name, symbol, totalSupply] = await Promise.all([nameResult.then(x => x[0]), symbolResult.then(x => x[0]), totalSupplyResult.then(x => BigInt(x[0].toString()))]);
      ...
      let [nameResult, symbolResult, decimalsResult, totalSupplyResult, versionResult] = await this._batchCallMethods([{
        address,
        abi: Solidity.qrc20ABIs.find(abi => abi.name === 'name')
      }, {
        address,
        abi: Solidity.qrc20ABIs.find(abi => abi.name === 'symbol')
      }, {
        address,
        abi: Solidity.qrc20ABIs.find(abi => abi.name === 'decimals')
      }, {
        address,
        abi: Solidity.qrc20ABIs.find(abi => abi.name === 'totalSupply')
      }, {
        address,
        abi: Solidity.qrc20ABIs.find(abi => abi.name === 'version')
      }]);
      try {
        let version;
        try {
  ...
  async _callMethod(address, abi, ...args) {
    let {
      executionResult
    } = await this.node.getRpcClient().callcontract(address.toString('hex'), Buffer.concat([abi.id, abi.encodeInputs(args)]).toString('hex'));
  ...
  async _batchCallMethods(callList) {
    ...
    let results = await client.batch(() => {
      for (let {
        ...
        client.callcontract(address.toString('hex'), Buffer.concat([abi.id, abi.encodeInputs(args)]).toString('hex'));
      }
    });
  ...
  async _updateBalances(balanceChanges) {
    ...
    let batchCalls = balanceChanges.map(({
      contract,
      address
    }) => ({
    ...
    let operations = await Promise.all(balanceChanges.map(async ({
      contract,
      address
    }, index) => {
      try {
    ...
    if (operations.length) {
      await this.#QRC20Balance.bulkCreate(operations, {
...
function isQRC20(code) {
  return code.includes(balanceOfABI.id) && code.includes(transferABI.id) && code.includes(TransferABI.id);
}
function isQRC721(code) {
  return code.includes(balanceOfABI.id) && code.includes(ownerOfABI.id) && code.includes(TransferABI.id);
}
