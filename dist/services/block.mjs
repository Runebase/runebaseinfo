...
import { AsyncQueue } from '../utils.mjs';
const {
  gt: $gt,
  between: $between
} = Sequelize.Op;
class BlockService extends Service {
  ...
  async start() {
    ...
    if (tip.height > 0 && !(await this.#Block.findOne({
      where: {
        height: tip.height
      },
      attributes: ['height']
    ...
    this.#bus = this.node.openBus({
      remoteAddress: 'localhost-block'
    });
    if (!tip) {
      this.#tipResetNeeded = true;
      return;
    }
    await this.#Block.destroy({
      where: {
        height: {
          [$gt]: tip.height
        }
  ...
  async _loadRecentBlockHashes() {
    let hashes = (await this.#Block.findAll({
      where: {
        height: {
          [$between]: [this.#tip.height - this.#recentBlockHashesCount, this.#tip.height]
        }
      },
      attributes: ['hash'],
      order: [['height', 'ASC']]
    ...
    this.logger.info('Block Service: loaded:', this.#recentBlockHashes.size, 'hashes from the index');
  }
  async _getTimeSinceLastBlock() {
    let header = await this.#Header.findOne({
      where: {
        height: Math.max(this.#tip.height - 1, 0)
      },
  ...
  async _onReorg(blocks) {
    ...
    let {
      hash: targetHash
    } = await this.#Header.findByHeight(targetHeight, {
      attributes: ['hash']
    });
    try {
  ...
  async _findLatestValidBlockHeader() {
    if (this.#reorgToBlock) {
      let header = await this.#Header.findByHeight(this.#reorgToBlock, {
    ...
    let header;
    for (let i = 0; i <= this.#recentBlockHashes.size; ++i) {
      let currentHeader = await this.#Header.findByHash(blockServiceHash, {
        attributes: ['hash', 'height']
      });
    ...
    assert(header, ['Block Service: we could not locate any of our recent block hashes in the header service index.', 'Perhaps our header service synced to the wrong chain?'].join(' '));
    assert(header.height <= this.#tip.height, ['Block Service: we found a common ancestor header whose height was greater than our current tip.', 'This should be impossible'].join(' '));
    return header;
  }
  async _findBlocksToRemove(commonHeader) {
    ...
    for (let i = 0; i < this.#recentBlockHashes.size && Buffer.compare(hash, commonHeader.hash) !== 0; ++i) {
      blocks.push({
        height,
        hash
      });
  ...
  async _handleReorg() {
    ...
    assert(blocksToRemove.length > 0 && blocksToRemove.length <= this.#recentBlockHashes.size, 'Block Service: the number of blocks to remove looks incorrect');
    this.logger.warn('Block Service: chain reorganization detected, current height/hash:', `${this.#tip.height}/${this.#tip.hash.toString('hex')}`, 'common ancestor hash:', commonAncestorHeader.hash.toString('hex'), `at height: ${commonAncestorHeader.height}.`, 'There are:', blocksToRemove.length, 'block(s) to remove');
    await this._setTip({
  ...
  async __onBlock(rawBlock) {
    let header;
    do {
      header = await this.#Header.findByHash(rawBlock.hash, {
    ...
    let minerId = (await this.#TransactionOutput.findOne({
      where: {
        outputIndex: isProofOfStake ? 1 : 0
      },
  ...
  async _logSynced() {
    ...
    try {
      ...
      this.logger.info('Block Service: the best block hash is:', this.#tip.hash.toString('hex'), 'at height:', `${this.#tip.height}.`, 'Block interval:', diff);
    } catch (err) {
      this._handleError(err);
    }
  }
  ...
  async _sync() {
    ...
    try {
      let {
        targetHash,
        endHash
      } = await this.#header.getEndHash(this.#tip, this.#readAheadBlockCount);
  ...
  _logProgress() {
    ...
    this.logger.info('Block Service: download progress:', `${this.#tip.height}/${bestHeight} (${progress}%)`);
  }
}
