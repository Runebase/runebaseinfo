...
import { Address, Opcode, OutputScript } from '../lib/index.mjs';
...
class TransactionService extends Service {
  ...
  async start() {
    ...
    await this.#db.query(`
      ...
      WHERE output.transaction_id = input.output_id AND output.output_index = input.output_index AND input.block_height > :height
    `, {
      replacements: {
    ...
    await this.#EVMReceiptMapping.destroy({
      truncate: true
    });
  ...
  async onReorg(height) {
    await this.#db.query(`
      ...
      SET output.input_id = 0, output.input_index = 0xffffffff, output.input_height = NULL
      WHERE input.transaction_id = tx._id AND tx.block_height > :minHeight AND tx.index_in_block = 1
        AND output.transaction_id = input.output_id AND output.output_index = input.output_index
    `, {
      replacements: {
        minHeight: Math.max(height, this.chain.lastPoWBlockHeight)
      }
    });
    await this.#db.query(`
    ...
    await this.#db.query(`
      ...
      WHERE tx.block_height > :height AND tx.index_in_block < 2
        AND (tx.index_in_block = 0 OR tx.block_height > :lastPoWBlockHeight)
    `, {
      replacements: {
        height,
        lastPoWBlockHeight: this.chain.lastPoWBlockHeight
      }
    });
  ...
  async _processBlock(block) {
    ...
    if (this.#synced) {
      let mempoolTransactions = await this.#Transaction.findAll({
        where: {
          id: {
            [$in]: block.transactions.slice(block.height > this.chain.lastPoWBlockHeight ? 2 : 1).map(tx => tx.id)
          }
        },
        attributes: ['_id', 'id']
      });
      ...
      if (mempoolTransactions.length) {
        ...
        await Promise.all([this.#TransactionOutput.update({
        ...
        await this.#db.query(`
          UPDATE transaction_output output, transaction_input input
          SET output.input_height = :blockHeight
          WHERE input.transaction_id IN (:ids) AND output.transaction_id = input.output_id AND output.output_index = input.output_index
        `, {
          replacements: {
            blockHeight: block.height,
            ids: _ids
        ...
        await this.#db.query(`
          ...
          SET address.create_height = LEAST(address.create_height, :blockHeight)
          WHERE address._id = output.address_id AND output.transaction_id IN (:ids)
        `, {
          replacements: {
            blockHeight: block.height,
            ids: _ids
    ...
    await this.#Witness.bulkCreate(witnesses, {
      validate: false
    });
    let ids = (await this.#Transaction.findAll({
      where: {
        id: {
          [$in]: newTransactions.map(tx => tx.id)
        }
      },
      attributes: ['_id'],
      order: [['_id', 'ASC']]
  ...
  async processTxos(transactions) {
    ...
    for (let {
      type,
      data
    } of addressMap.values()) {
      addressItems.push({
        type: this.#Address.parseType(type),
        data
      });
    }
    if (addressItems.length) {
      let existingAddresses = await this.#Address.findAll({
        where: {
          [$or]: addressItems.map(({
            type,
            data
      ...
      for (let address of existingAddresses) {
        let key = `${address.data.toString('hex')}/${this.#Address.getType(address.getDataValue('type'))}`;
        let item = addressMap.get(key);
        if (item) {
          for (let [index, outputIndex] of item.indices) {
            addressIds[index][outputIndex] = address._id;
          }
    ...
    for (let {
      ...
      type,
      data
    } of await this.#Address.bulkCreate(newAddressItems, {
      validate: false
    })) {
    ...
    await Promise.all([this.#TransactionOutput.bulkCreate(outputTxos, {
      validate: false
    }), this.#TransactionInput.bulkCreate(inputTxos, {
      validate: false
    }), ...(mappingObjects.length ? [this.#TransactionOutputMapping.bulkCreate(mappingObjects, {
      validate: false
    })] : [])]);
    await this.#db.query(`
  ...
  async processBalanceChanges({
    ...
    if (transactions) {
      ...
      if (block) {
        await this.#db.query(`
          UPDATE balance_change balance, transaction tx
          SET balance.block_height = :blockHeight, balance.index_in_block = tx.index_in_block
          WHERE tx._id = balance.transaction_id AND tx.block_height = :blockHeight
        `, {
          replacements: {
            blockHeight: block.height
          }
      ...
      filter = 'transaction_id BETWEEN :minId AND :maxId';
      replacements = {
        minId: transactions[0]._id,
        maxId: transactions[transactions.length - 1]._id
      };
    } else {
      filter = 'block_height = :blockHeight';
      replacements = {
        blockHeight: block.height
      };
    }
  ...
  async processReceipts(transactions) {
    ...
    for (let tx of transactions) {
      for (let outputIndex = 0; outputIndex < tx.outputs.length; ++outputIndex) {
        let output = tx.outputs[outputIndex];
        if ([OutputScript.EVM_CONTRACT_CREATE, OutputScript.EVM_CONTRACT_CREATE_SENDER, OutputScript.EVM_CONTRACT_CALL, OutputScript.EVM_CONTRACT_CALL_SENDER].includes(output.scriptPubKey.type)) {
          let senderType;
          let senderData;
          let hasOpSender = [OutputScript.EVM_CONTRACT_CREATE_SENDER, OutputScript.EVM_CONTRACT_CALL_SENDER].includes(output.scriptPubKey.type);
          if (hasOpSender) {
            ...
            let {
              address: refunder
            } = await this.#TransactionInput.findOne({
              where: {
                transactionId: tx._id,
                inputIndex: 0
              },
    ...
    if (receipts.length) {
      await this.#EVMReceipt.bulkCreate(receipts, {
        validate: false
      });
    }
  }
  async _processContracts(block) {
    ...
    for (let i = 0; i < block.transactions.length; ++i) {
      ...
      if (tx.inputs[0].scriptSig.length === 1 && tx.inputs[0].scriptSig[0] === Opcode.OP_SPEND) {
        ...
        if (tx.outputs.some(output => [OutputScript.EVM_CONTRACT_CREATE, OutputScript.EVM_CONTRACT_CREATE_SENDER, OutputScript.EVM_CONTRACT_CALL, OutputScript.EVM_CONTRACT_CALL_SENDER].includes(output.scriptPubKey.type))) {
    ...
    let refundTxos = await this.#TransactionOutput.findAll({
      where: {
        outputIndex: {
          [$gt]: block.header.isProofOfStake() ? 1 : 0
        }
      },
    ...
    let refunderMap = new Map((await this.#TransactionInput.findAll({
      ...
      include: [{
        ...
        where: {
          id: {
            [$in]: receiptIndices.map(index => block.transactions[index].id)
          }
        },
        attributes: ['id']
      }, {
    ...
    })).map(item => [item.transaction.id.toString('hex'), {
    ...
    for (let index = 0; index < receiptIndices.length; ++index) {
      ...
      for (let i = 0; i < tx.outputs.length; ++i) {
        if ([OutputScript.EVM_CONTRACT_CREATE, OutputScript.EVM_CONTRACT_CREATE_SENDER, OutputScript.EVM_CONTRACT_CALL, OutputScript.EVM_CONTRACT_CALL_SENDER].includes(tx.outputs[i].scriptPubKey.type)) {
          indices.push(i);
        }
      }
    ...
    await Promise.all([this.#GasRefund.bulkCreate(gasRefunds, {
      validate: false
    }), this.#EVMReceiptMapping.bulkCreate(receipts, {
      validate: false
    })]);
    await this.#db.query(`
      UPDATE evm_receipt receipt, evm_receipt_mapping mapping
      SET receipt.block_height = :blockHeight, receipt.index_in_block = mapping.index_in_block,
    ...
    await this.#EVMReceiptLog.bulkCreate(receiptLogs, {
      validate: false
    });
  }
  async removeReplacedTransactions(tx) {
    let prevTxs = await this.#Transaction.findAll({
      where: {
        id: {
          [$in]: tx.inputs.map(input => input.prevTxId)
        }
      },
      attributes: ['_id', 'id']
    });
    ...
    let transactionsToRemove = (await this.#TransactionOutput.findAll({
      ...
      attributes: [[Sequelize.fn('DISTINCT', Sequelize.col('input_id')), 'inputId']]
    })).map(tx => tx.inputId);
  ...
  async _removeMempoolTransaction(id) {
    ...
    let inputs = await this.#TransactionInput.findAll({
      where: {
        transactionId: id
      },
      attributes: ['outputId', 'outputIndex']
    });
    if (inputs.length) {
      await this.#TransactionOutput.update({
        inputId: 0,
        ...
        where: {
          [$or]: inputs.map(i => ({
            transactionId: i.outputId,
            outputIndex: i.outputIndex
          }))
        }
      });
    }
    await Promise.all([this.#TransactionOutput.destroy({
      where: {
        transactionId: id
      }
    }), this.#TransactionInput.destroy({
      where: {
        transactionId: id
      }
    }), this.#BalanceChange.destroy({
      where: {
        transactionId: id
      }
    })]);
    let txRecord = await this.#Transaction.findOne({
      where: {
        _id: id
      },
      attributes: ['id']
