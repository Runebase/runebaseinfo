...
class HeaderService extends Service {
  ...
  async start() {
    ...
    this.#bus = this.node.openBus({
      remoteAddress: 'localhost-header'
    });
  }
  ...
  async _setGenesisBlock() {
    assert(Buffer.compare(this.#tip.hash, this.#genesisHeader.hash) === 0, 'Expected tip hash to be genesis hash, but it was not');
  ...
  _queueBlock(block) {
    this.#blockProcessor.push(block, err => {
      if (err) {
        ...
        this.logger.debug(`Header Service: completed processing block: ${block.hash.toString('hex')},`, 'prev hash:', block.header.prevHash.toString('hex'));
      }
    });
  }
  ...
  async _handleReorg(block) {
    this.logger.warn(`Header Service: reorganization detected, current tip hash: ${this.#tip.hash.toString('hex')},`, 'new block causing the reorg:', block.hash.toString('hex'));
    this._adjustTipBackToCheckpoint();
    await this._adjustHeadersForCheckpointTip();
  }
  ...
  _logProcess() {
    ...
    this.logger.info('Header Service: download progress:', `${this.#tip.height}/${bestHeight}`, `(${progress}%)`);
    this.#lastTipHeightReported = this.#tip.height;
  }
  _getP2PHeaders(hash) {
    this.node.getHeaders({
      startHash: hash
    });
  }
  _sync() {
