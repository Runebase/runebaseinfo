...
class MempoolService extends Service {
  ...
  _startSubscriptions() {
    ...
    if (!this.#bus) {
      this.#bus = this.node.openBus({
        remoteAddress: 'localhost-mempool'
      });
    }
  ...
  async _onTransaction(tx) {
    ...
    try {
      ...
      await Promise.all([this.#Witness.bulkCreate(witnesses, {
        validate: false
      }), this.#transaction.processTxos([tx])]);
      await this.#transaction.processBalanceChanges({
        transactions: [tx]
      });
  ...
  async _validate(tx) {
    ...
    let count = await this.node.getModel('transaction_output').count({
      where: {
        [$or]: txos.map(([transactionId, outputIndex]) => ({
          transactionId,
          outputIndex
        }))
      }
