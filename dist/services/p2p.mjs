...
import { Pool, Messages, Inventory } from '../p2p/index.mjs';
import Service from './base.mjs';
class P2PService extends Service {
  #outgoingTransactions = new LRUCache({
    max: 100
  });
  #blockCache = null;
  #pool = null;
  ...
  _initP2P() {
    ...
    this.#configPeers = [{
      ...
      port: parseInt(process.env.P2P_PORT) || 9433
    }];
  ...
  _onPeerGetData(peer, message) {
    let txId = Buffer.from(message.inventory[0].data).reverse().toString('hex');
    let tx = this.#outgoingTransactions.get(txId);
    if (tx) {
      peer.sendMessage(this.#messages.tx({
        transaction: tx
      }));
    }
  }
  ...
  _onPeerInventories(peer, message) {
    ...
    for (let inventory of message.inventories) {
      if (!this.#inventories.get(inventory.data.toString('hex'))) {
        ...
        if ([Inventory.types.TRANSACTION, Inventory.types.BLOCK, Inventory.types.FILTERED_BLOCK].includes(inventory.type)) {
    ...
    if (newDataNeeded.length > 0) {
      peer.sendMessage(this.#messages.getdata({
  ...
  _matchChain(chain) {
    ...
    this.logger.error(`P2P Service: configured chain: "${this.chain.name}"`, `does not match our peer's reported network: "${chain.name}"`);
    this.node.stop();
  }
  _onPeerReady(peer, address) {
    ...
    this.logger.info(`Connected to peer: ${address.ip.v4},`, `chain: ${peer.chain.name}, version: ${peer.version},`, `subversion: ${peer.subversion},`, `status: ${peer.status},`, `port: ${peer.port},`, `best height: ${peer.bestHeight}`);
    this._addPeer(peer);
  ...
  _setResourceFilter(filter) {
    return {
      starts: [filter.startHash],
      stop: filter.endHash || 0
    };
  }
}
