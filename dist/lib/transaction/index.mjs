...
class Transaction {
  ...
  isCoinbase() {
    return this.inputs.length === 1 && Buffer.compare(this.inputs[0].prevTxId, Buffer.alloc(32)) === 0 && this.outputs.length > 0;
  }
  isCoinstake() {
    return this.inputs.length > 0 && Buffer.compare(this.inputs[0].prevTxId, Buffer.alloc(32)) !== 0 && this.outputs.length >= 2 && this.outputs[0].isEmpty();
  }
}
