...
import Script, { InvalidScriptError } from './index.mjs';
import OutputScript from './output.mjs';
const TYPES = {
  UNKNOWN: 'nonstandard',
  COINBASE: 'coinbase',
  PUBKEY: 'pubkey',
...
class InputScript extends Script {
  ...
  static fromBuffer(buffer, {
    ...
    if (witness.length) {
      for (const Class of [WitnessV0KeyHashInputScript, WitnessV0ScriptHashInputScript]) {
        if (Class.isValid(chunks, scriptPubKey, witness)) {
          return new Class(chunks, scriptPubKey, witness);
        }
      }
    } else {
      for (const Class of [PublicKeyInputScript, PublicKeyHashInputScript, MultisigInputScript, ContractSpendScript]) {
...
class CoinbaseScript extends InputScript {
  ...
  toString() {
    if (this.parsedChunks) {
      let chunks = this.parsedChunks.map(({
        code,
        buffer
      }) => {
        if (buffer) {
      ...
      if (code.isSmallInt()) {
        ...
        chunks[0] = Number.parseInt(Buffer.from(this.parsedChunks[0].buffer, 'hex').reverse().toString('hex'), 16);
      }
      return chunks.join(' ');
    } else {
...
class ScriptHashInputScript extends InputScript {
  ...
  toString() {
    ...
    switch (this.redeemScript.type) {
      ...
      default:
        return [...this.chunks.slice(0, -1).map(({
          code,
          buffer
        }) => {
          if (buffer) {
...
class ContractSpendScript extends InputScript {
  static isValid(chunks, scriptPubKey) {
    return [OutputScript.EVM_CONTRACT_CALL, OutputScript.EVM_CONTRACT_CALL_SENDER, OutputScript.CONTRACT].includes(scriptPubKey.type) && chunks.length === 1 && chunks[0].code === Opcode.OP_SPEND;
  }
  get type() {
...
export { CoinbaseScript, PublicKeyInputScript, PublicKeyHashInputScript, ScriptHashInputScript, MultisigInputScript, WitnessV0KeyHashInputScript, WitnessV0ScriptHashInputScript, ContractSpendScript };
...
export { UNKNOWN, COINBASE, PUBKEY, PUBKEYHASH, SCRIPTHASH, SCRIPTHASH_WRAP_MULTISIG, MULTISIG, SCRIPTHASH_WRAP_WITNESS_V0_KEYHASH, SCRIPTHASH_WRAP_WITNESS_V0_SCRIPTHASH, SCRIPTHASH_WRAP_WITNESS_V0_SCRIPTHASH_WRAP_MULTISIG, WITNESS_V0_KEYHASH, WITNESS_V0_SCRIPTHASH, CONTRACT_SPEND };