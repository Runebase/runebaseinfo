...
const qrc20List = JSON.parse(readFileSync(new URL('./qrc20-abi.json', import.meta.url)));
const qrc721List = JSON.parse(readFileSync(new URL('./qrc721-abi.json', import.meta.url)));
function getTypes(abi, category) {
  let result = [];
  for (let item of abi[category]) {
    if (item.type === 'tuple') {
      result.push(`(${getTypes({
...
class EventABI {
  ...
  encode(params) {
    ...
    for (let index = 0; index < this.inputs.length; ++index) {
      ...
      if (input.indexed) {
        topics.push(ethjsABI.rawEncode(getTypes({
          inputs: [input]
        }, 'inputs'), [params[index]]));
      } else {
    ...
    let data = ethjsABI.rawEncode(getTypes({
      inputs: unindexedInputs
    }, 'inputs'), unindexedParams);
    return {
      topics,
      data
    };
  ...
  decode({
    ...
    for (let index = 0; index < topics.length; ++index) {
      ...
      let [param] = ethjsABI.rawDecode(getTypes({
        inputs: [input]
      }, 'inputs'), topics[index]);
      indexedParams.push(param);
    }
    let unindexedParams = ethjsABI.rawDecode(getTypes({
      inputs: unindexedInputs
    }, 'inputs'), data);
    let params = [];
...
export { MethodABI, EventABI, qrc20ABIs, qrc721ABIs };