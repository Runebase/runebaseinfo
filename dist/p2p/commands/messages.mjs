import { Hash } from '../../lib/index.mjs';
import messageMap from './commands/index.mjs';
...
class Messages {
  ...
  constructor(options) {
    ...
    for (let [command, Command] of Object.entries(messageMap)) {
      this[command] = this.#commands[command] = (args = {}) => new Command({
        ...args,
        ...options
      });
    }
  ...
  parseBuffer(buffer) {
    ...
    let command = buffer.slice(4, 16).toString('ascii').replace(/\0+$/, '');
