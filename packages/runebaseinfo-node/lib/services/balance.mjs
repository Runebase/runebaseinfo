...
export default class BalanceService extends Service {
  ...
  async onBlock(block) {
    ...
    if (block.height === 0) {
      ...
      await RunebaseBalance.insertMany(
    ...
    await RunebaseBalance.collection.bulkWrite(
  ...
  async onReorg(height, hash) {
    this._processing = true
    let balanceChanges = await RunebaseBalanceChanges.aggregate([
      {
        $match: {
    ...
    await RunebaseBalance.deleteMany({height: {$gt: height}})
  ...
  async _rebuildBalances() {
    this._processing = true
    await RunebaseBalance.deleteMany({height: {$gt: this._tip.height}})
