import {Address, Script, Solidity} from 'runebaseinfo-lib'
...
export default class AddressService extends Service {
  ...
  async getAddressBalanceHistory(
    ...
    addresses = parseAddresses(addresses)
    let sort = reversed ? {'block.height': -1, index: -1} : {'block.height': 1, index: 1}
    let count = await RunebaseBalanceChanges.countDocuments({address: {$in: addresses}})
    let list = await RunebaseBalanceChanges.aggregate([
    ...
    let [initialBalance] = await RunebaseBalanceChanges.aggregate([
      {
        $match: {
  ...
  async getAddressSummary(addresses) {
    addresses = parseAddresses(addresses)
    let [balanceChangesResult] = await RunebaseBalanceChanges.aggregate([
      {$match: {address: {$in: addresses}}},
      {
        $group: {
          _id: null,
