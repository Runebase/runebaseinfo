import {Address, Script, Input, Output, Transaction as RawTransaction} from 'runebaseinfo-lib'
import Transaction from '../models/transaction'
import TransactionOutput from '../models/transaction-output'
import RunebaseBalanceChanges from '../models/runebase-balance-changes'
...
export default class TransactionService extends Service {
  ...
  async getBlockReward(height, isProofOfStake = true) {
    ...
    let [result] = await RunebaseBalanceChanges.aggregate([
      {
        $match: {
          'block.height': height,
  ...
  async start() {
    ...
    await RunebaseBalanceChanges.deleteMany({'block.height': {$gt: this._tip.height}})
  ...
  async onReorg(height) {
    ...
    await RunebaseBalanceChanges.updateMany(
  ...
  async _processTransaction(tx, indexInBlock, block) {
    ...
    if (block.height > 0) {
      ...
      await RunebaseBalanceChanges.collection.insertMany(balanceChanges, {ordered: false})
      tx.balanceChanges = balanceChanges
        .filter(item => item.address)
        .map(({address, value}) => ({
          addressKey: `${address.type}/${address.hex}`,
